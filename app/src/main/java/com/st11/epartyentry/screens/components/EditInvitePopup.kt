...
@Composable
fun EditInvitePopup(onDismiss: () -> Unit, inviteId: String) {
    ...
    val inviteeState by inviteeViewModel.inviteeState.collectAsState()

// Fetch user details if inviteId is not null
    LaunchedEffect(inviteId) {
        inviteId.let { inviteeViewModel.getInviteeDetails(it) }
    }

    LaunchedEffect(inviteeState) {
        inviteeState?.let {
                ...
                try {
                    ...
                    val parsed = phoneUtil.parse(it.phone, null)

                    countryCode = "+${parsed.countryCode}"
                    phoneNumber = parsed.nationalNumber.toString()

                    // Find country name from your countries list
                    val match = countries.find { c -> c.code == countryCode }
                    if (match != null) {
...
//                    inviteeViewModel.insertInvitee(invitee = InviteeEntity())
                    CoroutineScope(Dispatchers.Main).launch {
                        inviteeViewModel.updateInviteeById(fullName, fullPhoneNumber, inviteId)
                    }
                    onDismiss()
                }else{
                ...
                Text("Update", color = colorResource(R.color.green))
            }
        },
        dismissButton = {
